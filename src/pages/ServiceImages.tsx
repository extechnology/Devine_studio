...
const ServiceImages = () => {
  ...
  const serviceImagesData = serviceCategory?.find(
    (item) => item.name === category,
  );

  const allImages = serviceImagesData?.images ?? [];
  ...
  return (
    <div className="bg-charcoal min-h-screen text-white font-body selection:bg-accent selection:text-white pb-20">
      {/* New Service Images Paginated Gallery Section */}
      <ScrollReveal
        threshold={0.05}
        delay={100}
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5 border-t border-l border-white/20">
          {currentItems.length > 0 ? (
            ...
            <div className="col-span-3 py-24 text-center text-white/30 font-heading tracking-widest uppercase text-sm">
              No images found
            </div>
          )}
        </div>
