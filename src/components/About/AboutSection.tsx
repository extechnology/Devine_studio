...
const AboutCard = ({
  ...
  const previewText = description.slice(0, 180) + "...";

  return (
    <div className="relative w-full">
      {/* IMAGE */}
      <div
        className={`relative w-full md:w-[65%] group overflow-hidden ${
          reverse ? "md:ml-auto" : ""
        }`}
      >
        <img
...
const AboutSection = () => {
  const { data: aboutContents, isLoading } = useAboutContents();
    ...
    return (
      <div className="space-y-20">
        {[...Array(3)].map((_, i) => (
          <div
            key={i}
            className="relative w-full h-[350px] md:h-[450px] bg-white/5 animate-pulse"
          />
        ))}
      </div>
    );
  ...
  if (!aboutContents?.length) return null;
